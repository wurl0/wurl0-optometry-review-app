...
D={}
for f in sorted(glob.glob(SC+"/attach_B_*.json")):
    part=json.load(open(f))
    for k in part:
        assert k not in D, ("dup", k)
    D.update(part)
print("authored:", len(D))
A=json.load(open("banks/B.json"))
ids={q['id'] for q in A}
...
if not missing and not extra and not bad:
    ...
    open("banks/B.json","w").write(json.dumps(A, indent=1, ensure_ascii=False)+"\n")
    print("APPLIED decode+ow to", n, "B questions")

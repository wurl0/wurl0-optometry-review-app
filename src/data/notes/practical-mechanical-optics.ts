      ...
      blocks: [
        ...
        },
        {
          ...
          blocks: [
            ...
            {
              kind: 'table',
              headers: ['Element', 'Transmission (T)', 'Notes'],
              rows: [
                ['AR-coated surface', '99%', 'Anti-reflective treatment; the highest single-surface value'],
                ['Mirror-coated surface', '50%', 'Reflects half the incident light'],
                ['Uncoated CR-39 surface (n = 1.49)', '96.13%', 'From R = [(1.49−1)/(1.49+1)]² × 100 = 3.87%'],
                ['Uncoated crown glass surface (n = 1.523)', '95.70%', 'Higher index reflects more, so transmits slightly less'],
                ['Uncoated Trivex surface (n = 1.53)', '95.61%', 'R = (0.53/2.53)² × 100 = 4.39%'],
                ['Uncoated polycarbonate surface (n = 1.59)', '94.81%', 'R = (0.59/2.59)² × 100 = 5.19%'],
                ['Tint A (light)', '85%', 'Absorption through the lens material'],
                ['Tint B (medium)', '65%', 'Absorption through the lens material'],
                ['Tint C (dark)', '35%', 'Absorption through the lens material'],
              ],
            },
            {
              kind: 'bullets',
              items: [
                'Uncoated single-surface transmission depends on index: T = 100 − R with R = [(n−1)/(n+1)]² × 100 (air n = 1.00)',
                'Multiply every element in the light path: each surface, tint, mirror, and AR coating contributes its own transmission factor',
                'Worked example (uncoated CR-39, two surfaces): T_total = 0.9613 × 0.9613 = 0.9241, so T = 92.40%',
                'Worked example (AR front + uncoated back on CR-39): T_total = 0.99 × 0.9613 = 0.9517, so T = 95.17%',
                'Optical density example: a system with T = 0.64 gives OD = log₁₀(1 / 0.64) = 0.194',
              ],
            },
            {
              kind: 'pearl',
              content: 'Read the setup element by element and multiply. AR raises transmission (99% per treated surface); mirror halves it (50%); tints cut it by grade (A 85%, B 65%, C 35%); each uncoated surface passes roughly 94 to 96% depending on index. Because factors multiply, adding any absorbing element always lowers T_total, while optical density (log of 1/T) simply adds up.',
            },
          ],
        },
        {
